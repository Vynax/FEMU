...
#define OC20_LBA_GET_SECTR(lbaf, lba) \
    ((lba & (lbaf)->sec_mask) >> (lbaf)->sec_offset)

#define OC20_LBA_GET_CHUNK(lbaf, lba) \
    ((lba & (lbaf)->chk_mask) >> (lbaf)->chk_offset)

#define OC20_LBA_GET_PUNIT(lbaf, lba) \
    ((lba & (lbaf)->lun_mask) >> (lbaf)->lun_offset)

#define OC20_LBA_GET_GROUP(lbaf, lba) \
    (lba >> (lbaf)->grp_offset)

#define OC20_LBA(lbaf, group, punit, chunk, sectr) \
    (sectr << (lbaf)->sec_offset | chunk << (lbaf)->chk_offset | punit << (lbaf)->lun_offset | group << (lbaf)->grp_offset)
...
typedef struct abc
{
    ...
    int OC20_VECT_WRITE;
    int OC20_VECT_ERASE;
    struct N_Request NVME_Request;
    struct N_Cmd NVMe_Cmd;
} abc;
